...
const AnalysisProgress = ({ currentStep, error, onRetry, onCancel }: AnalysisProgressProps) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center py-24 md:py-28">
      ...
      {/* Secondary: Subtitle + micro-status */}
      <AnalyzingSubtitle />
      ...
      <div className="space-y-2.5 w-full max-w-[180px]">
        {steps.map((label, i) => {
          ...
          return (
            <div key={label} className="flex items-center gap-3 h-[20px]">
              <div
      ...
      {/* Quaternary footer: remaining time + cancel */}
      <div className="flex flex-col items-center mt-10 gap-3">
        {remainingStr && (
          <p className="font-mono-brand text-[10px] text-muted-foreground/35 tabular-nums tracking-wide">
            {remainingStr}
          </p>
        )}
        {onCancel && (
